      ...
      { page > 1 && (
        ...
        {pages.map((currentPage) => (
            <Link
                ...
                className={`${ page === currentPage ? 'font-black bg-amber-500' : 'bg-white'} px-4 py-2 test-sm test-gray-900 ring-1 ring-inset ring-gray-300 focus:z-20 focus:outline-offset-0`}
            >{currentPage}</Link>
        ))}
